...
class MobileHandPipeline:
    ...
    def __init__(self, model_path=None, max_hands=2, use_onnx=True, onnx_variant='int8', lite_mode=False):
        ...
        print(f"[Pipeline] Initializing Face Mesh (Lite={lite_mode})...")
        self.face_mesh = mp_face_mesh.FaceMesh(
            static_image_mode=False,
            max_num_faces=1,
            refine_landmarks=not lite_mode,  # Critical Memory Saving: Disable Iris model if lite_mode
    ...
    def _run_mobilehand_on_crop(self, crop, bbox):
        ...
        try:
            ...
            # MobileHand outputs keypoints with Y inverted (origin at bottom)
            # Flip Y to match image coordinates (origin at top)
            keypt_np[:, 1] = 224.0 - keypt_np[:, 1]
    ...
    def print_stats(self):
        ...
        print(f"  - Device: {self.device if self.device else 'N/A (ONNX direct)'}")
