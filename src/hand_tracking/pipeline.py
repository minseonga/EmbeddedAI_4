...
class HandTrackingPipeline:
    ...
    def __init__(self, precision: str = "fp32", prune_rate: float = 0.0):
        ...
        if prune_rate > 0:
            ...
            if precision == "int8":
                # Try Pruned + INT8 ONNX first
                model_path = ROOT / f"assets/models/yolo11n_hand_pose_pruned_{prune_pct}_int8.onnx"
                if not model_path.exists():
                     # Fallback to PT
                     print(f"[Warning] Pruned INT8 ONNX not found, trying FP32 PT")
            ...
            # Priority: TensorRT Engine -> ONNX INT8
            model_path = ROOT / "assets/models/yolo11n_hand_pose_int8.engine"
            if not model_path.exists():
                model_path = ROOT / "assets/models/yolo11n_hand_pose_int8.onnx"
                if not model_path.exists():
                     # Fallback to standard ONNX? or Base
                     model_path = ROOT / "assets/models/yolo11n_hand_pose.onnx"
            
            if not model_path.exists():
                print(f"[Warning] INT8 models not found, using FP32")
                model_path = ROOT / "assets/models/yolo11n_hand_pose.pt"
                
        elif precision == "fp16":
            model_path = ROOT / "assets/models/yolo11n_hand_pose_fp16.engine"
            if not model_path.exists():
                model_path = ROOT / "assets/models/yolo11n_hand_pose.onnx" # ONNX is often FP32 but better than PT
